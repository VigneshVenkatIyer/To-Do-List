...
const Todo = ({ todo, index, markTodo, removeTodo} : any) => {
    ...
    const showOptionButtons = (ele : any) =>{
        let btnParent = document.getElementsByClassName('buttonPrnt') as HTMLCollectionOf<HTMLElement>
        for(var i=0; i< btnParent.length; i ++){
          btnParent[i].classList.add("hide");
        } 
        let pNode = document.getElementById(ele.currentTarget.id)
        if((pNode as HTMLInputElement).checked === true){
